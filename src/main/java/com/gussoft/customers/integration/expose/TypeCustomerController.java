...
public class TypeCustomerController {
  ...
  @Autowired
  private TypeCustomerService service;

  @GetMapping("/customers/types")
  public Mono<ResponseEntity<Flux<TypeCustomerResponse>>> findAllCustomer() {
    return Mono.just(
      ResponseEntity.ok()
  ...
  @GetMapping("/customers/types/{id}")
  public Mono<ResponseEntity<TypeCustomerResponse>> findById(@PathVariable String id) {
    return service.findById(id).map(ResponseEntity::ok).defaultIfEmpty(ResponseEntity.notFound().build());
  }

  @PostMapping(path = "/customers/types")
  public Mono<ResponseEntity<TypeCustomerResponse>> create(@RequestBody Mono<TypeCustomerRequest> request) {
    return request.flatMap(customer -> {
      return service.save(Mono.just(customer)).map(types -> ResponseEntity
  ...
  @DeleteMapping("/customers/types/{id}")
